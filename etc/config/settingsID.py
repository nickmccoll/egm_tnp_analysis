...
flags = {
    ...
    'passingTight80XIP'       : '((passingTight80X == 1)&&(abs(el_dxy)<0.05)&&(abs(el_dxy)<0.05)&&(abs(el_dz)<0.1)&&(abs(el_sip3D)<4))',
    'passingTight80XNoIsoIP'       : '((passingTight80XNoIso== 1)&&(abs(el_dxy)<0.05)&&(abs(el_dxy)<0.05)&&(abs(el_dz)<0.1)&&(abs(el_sip3D)<4))',
    'passingTight80XNoIso'       : '((passingTight80XNoIso== 1))',
    }


baseOutDir = 'results/id/' 
...
samplesDef = {
    'data'   : tnpSamples.Moriond17_80X_id['data'].clone(),
    'mcNom'  : tnpSamples.Moriond17_80X_id['DY_madgraph'].clone(),
    'mcAlt'  : tnpSamples.Moriond17_80X_id['DY_amcatnlo'].clone(),
    'tagSel' : tnpSamples.Moriond17_80X_id['DY_madgraph'].clone(),
}
...
biningDef = [
   ...
   { 'var' : 'el_pt'  , 'type': 'float', 'bins': [10,20,35,50,90,500] },
]
...
#############################################################
### cut
# cutBase   = 'tag_Ele_pt > 27 && abs(tag_sc_eta) < 2.17 && el_q*tag_Ele_q < 0';

#running on skim
cutBase   = 'tag_Ele_pt >27'
...
tnpParNomFit = [
    ...
    "meanF[-0.0,-5,5.0]","sigmaF[0.9,0.5,5.0]",
